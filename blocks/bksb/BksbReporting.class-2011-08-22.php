	...
	******************************************************************/
    class BksbReporting 
    {
        ...
        public function __construct() {
			...
			/* Connect to old database to fix usernames */
			/*
            $this->old_selected_db = "bksb_mini_old";
            // create an instance of the  ADO connection object
            $this->old_connection = new COM ("ADODB.Connection") or die("Cannot start ADO");
            // define connection string, specify database driver
            $old_con = "PROVIDER=SQLOLEDB;SERVER=".$this->server.";UID=".$this->user.";PWD=".$this->password.";DATABASE=".$this->old_selected_db;
            $this->old_connection->open($old_con); //Open the connection to the database
			*/
		...
		*/
        ...
        public function getAllResults($user_id='') {
            ...
            if ($user_id != '') {
                ...
                $query = "SELECT Result FROM dbo.bksb_IAResults WHERE UserName = '$user_id'";
                ...
                $query = "SELECT WordProcessing, Spreadsheets, Databases, DesktopPublishing, Presentation, Email, General, Internet FROM dbo.bksb_ICTIAResults WHERE UserName = '$user_id'";
                if ($result = $this->connection->execute($query)) {
                    while (!$result->EOF) {
		...
		*/
		public function updateInvalidUsers($invalid_users = '') {
		
			if ($invalid_users != '' && is_array($invalid_users)) {
				$no_users_updated = 0;
				...
				foreach ($invalid_users as $key => $user) {
					...
					if ($user_match = get_record('user', 'firstname', $user['firstname'], 'lastname', $user['lastname'])) {
			...
			if ($no_users_updated > 0) {
				...
				// Remove duplicates
				$this->removeDuplicateUsers(TRUE);
			}
		}
		...
		public function getInvalidBksbUsers($firstname='', $lastname='', $order_field='userName') {
			...
			$valid_orders = array('userName', 'FirstName', 'LastName');
			...
			if ($firstname != '' && $lastname != '') {
				...
				$query = "SELECT user_id, userName, FirstName, LastName, DOB, PostcodeA as Postcode FROM dbo.bksb_Users ORDER BY $order ASC";
			}
					...
					*/
					...
					if ($invalid === TRUE) {
						$invalid_users[] = array(
									...
									'postcode' => ($result->fields['Postcode']->value != '') ? $result->fields['Postcode']->value : "",
		...
		public function getIAForGroup($group_name='', $ia_type = '') {
			...
			if ($group_name != '' && $ia_type != '') {
				...
				if ($ia_type == 'English' || $ia_type == 'Mathematics') {
					...
					$query = "SELECT UserName, Result FROM dbo.bksb_IAResults WHERE (UserName IN (SELECT UserName FROM dbo.bksb_GroupMembership WHERE (group_id = '$group_name'))) ORDER BY UserName";
					...
					$query = "SELECT UserName, WordProcessing, Spreadsheets, Databases, DesktopPublishing, Presentation, Email, General, Internet FROM dbo.bksb_ICTIAResults WHERE (UserName IN (SELECT UserName FROM bksb_GroupMembership WHERE (group_id = '$group_name'))) ORDER BY UserName";
		...
		public function getIctTotals($users='') {
		...
		public function removeDuplicateUsers($notify = FALSE) {
			...
			if (count($duplicate_users) > 0) {
				...
				foreach ($duplicate_users as $user) {
					...
					$query2 = "SELECT user_id, userName, FirstName, LastName FROM dbo.bksb_Users WHERE userName = '".$user['username']."' ORDER BY user_id ASC";
		...
		public function updateBksbData($old_username='', $new_username='', $firstname='', $lastname='') {
			if ($old_username != '' && $new_username != '' && $firstname != '' && $lastname != '') {
				...
				$this->removeDuplicateUsers(FALSE);
