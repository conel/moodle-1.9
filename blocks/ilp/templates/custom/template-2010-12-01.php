...
if($CFG->ilpconcern_status_per_student == 1){
	...
	switch ($studentstatusnum) {
		case 0:
			$status_icon = '<img src="/VLE/blocks/ilp/templates/custom/pix/status_green.jpg" alt="" width="30" height="30" />';
			break;	
		case 1:
			$status_icon = '<img src="/VLE/blocks/ilp/templates/custom/pix/status_amber.jpg" alt="" width="30" height="30" />';
			break;
		case 2:
			$status_icon = '<img src="/VLE/blocks/ilp/templates/custom/pix/status_red.jpg" alt="" width="30" height="30" />';
			break;
		case 3:
			$status_icon = '<img src="/VLE/blocks/ilp/templates/custom/pix/status_withdrawn.jpg" alt="" width="30" height="30" />';
			break;
	}
				...
				<tr>
					...
					<?php
						...
						<div class="add_button">
							<?php echo '<form method="post" action="https://clg.conel.ac.uk/VLE/blocks/lpr/actions/export.php?learner_id='.$user->id.'&amp;single=1">';?>
    ...
    <tr>
		...
		<td style="vertical-align:top;">
			<div id="attendance_icon">
					<?php
						$icon_green = '<img src="/VLE/blocks/ilp/templates/custom/pix/status_green.jpg" alt="" width="30" height="30" />';
						$icon_orange = '<img src="/VLE/blocks/ilp/templates/custom/pix/status_amber.jpg" alt="" width="30" height="30" />';
						$icon_red = '<img src="/VLE/blocks/ilp/templates/custom/pix/status_red.jpg" alt="" width="30" height="30" />';
...
<?php 
			...
			if($CFG->ilpconcern_report4 == 1 && (has_capability('mod/ilpconcern:addreport4', $context) || ($USER->id == $user->id && has_capability('mod/ilpconcern:addownreport4', $context)))) {
				echo '<form method="post" action="https://clg.conel.ac.uk/VLE/mod/ilpconcern/concerns_view.php?courseid='.$courseid.'&amp;userid='.$user->id.'&amp;action=updateconcern&amp;status=3">';
...
echo '<h2><img alt="" src="https://clg.conel.ac.uk/VLE/blocks/ilp/templates/custom/pix/report1.gif" /> <a href="https://clg.conel.ac.uk/VLE/blocks/ilp/attendance.php?courseid='.$courseid.'&amp;userid='.$user->id.'">Attendance</a></h2>';
echo '</div>';
...
if ($config->ilp_show_targets == 1) {
	...
	if(has_capability('mod/ilptarget:addtarget', $context) || ($USER->id == $user->id && has_capability('mod/ilptarget:addowntarget', $context))) {
		...
		echo '<form action="https://clg.conel.ac.uk/VLE/mod/ilptarget/target_view.php?courseid='.$courseid.'&amp;userid='.$user->id.'&amp;action=updatetarget" method="post">';
		echo '<input type="submit" value="Add Target" />';
		echo '</form>';
		echo '</div>';
	}
	...
	*/
	...
	if(eval('return $CFG->ilpconcern_report'.$i.';') == 1) {
		if($CFG->ilpconcern_report1 == 1 && (has_capability('mod/ilpconcern:addreport1', $context) || ($USER->id == $user->id && has_capability('mod/ilpconcern:addownreport1', $context)))) {
			echo '<div class="add_button">';
			echo '<form action="https://clg.conel.ac.uk/VLE/mod/ilpconcern/concerns_view.php?courseid='.$courseid.'&amp;userid='.$user->id.'&amp;action=updateconcern&amp;status=0" method="post">';
...
if ($config->ilp_show_lprs == 1) {
    ...
    echo '<div class="generalbox" id="ilp-lprs-overview">';
		...
		if ($can_write) {
			echo '<div class="add_button">';
			echo '<form method="post" action="https://clg.conel.ac.uk/VLE/blocks/lpr/actions/create.php?course_id='.$courseid.'&amp;ilp=1&amp;learner_id='.$user->id.'">
			<div><input type="submit" value="Add New Report" name="submit" /></div></form>';
			echo '</div>';
		}
