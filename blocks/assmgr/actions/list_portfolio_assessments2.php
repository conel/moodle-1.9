...
if(!empty($course)) {
	...
	// nkowald - 2011-11-02 - Updated link in page to link back to course page
	$old_link = $CFG->wwwroot."/blocks/assmgr/actions/list_portfolio_assessments.php?category_id={$category_id}";
	$course_link = $CFG->wwwroot . '/course/view.php?id='.$course_id;
    //$navlinks[] = array('name' => $category->name, 'link' => $old_link, 'type' => 'title');
    $navlinks[] = array('name' => $course->shortname, 'link' => "$course_link", 'type' => 'title');
    $navlinks[] = array('name' => 'Portfolios', 'link' => "", 'type' => 'title');
...
$PAGE->set_url('/blocks/assmgr/actions/list_portfolio_assessments2.php', $PARSER->get_params());

require_once($CFG->dirroot.'/blocks/assmgr/views/list_portfolio_assessments2.html');
