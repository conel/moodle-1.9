...
if(!empty($modules)){
	foreach($modules as $module_id => $choosen) {
		...
		// nkowald - 2011-10-10 - Should update every time, this was useful as it was used only when editing a current one. New ones were not updating module_complete
		if ($choosen == 1) {
...
if(empty($errors)) {
    ...
    header("Location: {$CFG->wwwroot}/blocks/lpr/actions/view.php?id={$lpr->id}&ilp={$ilp}&saved=1");
} else {
