...
if($risk) {
	...
	// nkowald - 2010-11-12 - Removed course id from here so it shows ALL LPRs - on the request of Scott
    //$lprs = $lpr_db->list_lprs($learner_id, $course_id, $table);
    $lprs = $lpr_db->list_lprs($learner_id, NULL, $table);
}
...
if(!empty($lprs)) {
    ...
    foreach ($lprs as $lpr) {
		...
		// nkowald - 2010-12-06 - Added check for deleted course
        $courselink = ($lpr->course_name != '') ? "<a href='{$CFG->wwwroot}/course/view.php?id={$lpr->course_id}'>{$lpr->course_name}</a>" : "<span style=\"color:red;\">Deleted Course</span>";
        ...
        $table->add_data(
            array(
                ...
                $profilelink,
                $courselink,
				str_replace('LPR', 'Target ', $lpr->lpr_name),
