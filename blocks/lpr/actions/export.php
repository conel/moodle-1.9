...
jQuery(document).ready(function() {
	jQuery("#pdf_export").submit(function(e) {
		// check if first name is blank
		var learner_id = jQuery("#menulearner_id").val();
		if (learner_id == 0) {
			alert('\'Learner\' is a required field');
			jQuery('#menulearner_id').focus();
			return false;
		}
		var email_add = jQuery("#email_field").val();
		if (email_add == '') {
			alert('\'Email To\' is a required field');
			jQuery('#email_field').focus();
			return false;
		}
	});
});
