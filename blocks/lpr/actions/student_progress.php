<?php
/**
 * Displays a table of Student Progress, filtered by optional parameters
...
$heading = implode(' - ', $heading).' : Student Progress';
...
$table->no_sorting('status_changes');
...
$status = POSTS_PROGRESS;
// get the list of at risk LPRs
$ilps = $lpr_db->get_concern_records($category_id, $course_id, $table, $start_timestamp, $end_timestamp, $status);
...
if(!empty($ilps)) {
    ...
    foreach ($ilps as $user) {
        ...
        $profilelink = '<strong><a href="'.$CFG->wwwroot.'/user/view.php?id='.$user->id.
            ...
            // nkowald - 2011-05-16 - Get how many times this was changed
			$query = "SELECT COUNT(history) as times_changed FROM mdl_ilpconcern_status WHERE userid = ".$user->id." AND created >= $start_timestamp AND created <= $end_timestamp";
			...
			$tutor_carried_out		=	$lpr_db->count_concern_posts(array("$user->id"=>$user->id),POSTS_PROGRESS,$category_id,$course_id,$start_timestamp, $end_timestamp,false);
...
require_once("{$CFG->dirroot}/blocks/lpr/views/student_progress.html");
