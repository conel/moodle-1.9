...
$student_status	=	 required_param('status',PARAM_INT);
...
$end_timestamp = !empty($end_timestamp) ? $end_timestamp : null;
...
switch($student_status)  {
	...
	case 0:
		$title = get_string('learnerswithgreen','block_lpr');
		break;
	case 1:
		$title = get_string('learnerswithamber','block_lpr');
		break;
	default:
		$title = get_string('learnerswithred','block_lpr');
		break;
	}
...
$status_list = $lpr_db->get_learners_status($student_status,$category_id, $course_id, $table, $start_timestamp, $end_timestamp);
...
require_once("{$CFG->dirroot}/blocks/lpr/views/student_status.html");
