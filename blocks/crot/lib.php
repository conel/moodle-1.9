...
/*
	for($i = 0; $i < $text_len; $i++){
		...
		$hash->value = hash('md5', mb_substr($stripped_text, $i, $gram_size, "utf-8"));
		$hash->position = $orig_positions[$i];
		$hashes[$i] = $hash;
	}
*/
	// TODO remove 4 lines below
/*	echo "+processing document of".$text_len." characters\n";
	echo "#size of hashes array is :".count($hashes)."\n";
	$cur_mem=memory_get_usage()-$cur_mem;
	echo ">total memory:".memory_get_usage()."\n_memory consumed $cur_mem\n";
*/	
		...
		switch (strtolower($path_parts['extension'])):
			case "doc":
				...
				if (strlen($infile)>0){
					...
					if (filesize($tmpfilename)<500000){
