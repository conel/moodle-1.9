...
function print_grade_plugin_selector($plugin_info, $return=false) {
	...
	// Hide outcomes from everyone except admins and elts
	$allowed = false;
	if ($roles = get_records('role_assignments', 'userid', $USER->id)) {
	...
	if (!$allowed) {
		unset($plugin_info['strings']['outcome']);
		unset($plugin_info['outcome']);
	}
