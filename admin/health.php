<?php  // $Id: health.php,v 1.17 2007-04-30 17:08:40 skodak Exp $
    ...
    $solution = optional_param('solution', 0, PARAM_SAFEDIR); //in fact it is class name alphanumeric and _
	...
	// nkowald - 2011-03-25 - No fucking idea why but health script stopped working and adding this made it work again.
	echo '&nbsp;';
...
function health_find_duplicates($table, $column1/*, $column2 ...  - you can add any # of columns on */) {
    // Find entries in the given table (don't include $CFG->prefix) where id1!=id2 but column1==column1 && column2==column2 ...
    // This function returns only duplicates 
    // (i.e. it ignores the entry with the lowest ID from any set of duplicates, which is presumably the correct original)
    // Rather than columns, you can also pass simple boolean statements as any argument after column1, such as 'type="course"'.
    // All entries must meet these additional conditions to be considered duplicates 
    global $CFG;
    $groups = $column1;
    $earlycheck = '';
    $where = "(t.$column1 = d.$column1 OR (t.$column1 IS NULL AND d.$column1 IS NULL))";
    for ($i=2;$i < func_num_args();$i++) {
        $column = func_get_arg($i);
        if (strpos($column, '=') !== FALSE || strpos($column, '<>') !== FALSE) {
            // We have not been given a column, but a boolean statement.
            // make sure all rows returned meet this statement
            if ($earlycheck == '') {
                $earlycheck .= ' WHERE ('.$column.')';
            } else {
                $earlycheck .= ' AND ('.$column.')';
            }
            $where .= ' AND (t.'.$column.')';
        } else {
            $groups .= ', '.$column;
            $where .= ' AND (t.'.$column.' = d.'.$column.' OR (t.'.$column.' IS NULL AND d.'.$column.' IS NULL))';
        }
    }
    //$dupes = get_records_sql("SELECT * FROM {$CFG->prefix}$table $earlycheck GROUP BY $groups HAVING COUNT(*) > 1");
    // ^- this finds duplicates, but only gives us the _lowest_ id of any duplicated entries. We want the highest. So:
    $temp_table = $CFG->prefix.'health_dupes';
    
    if (execute_sql("CREATE TEMPORARY TABLE $temp_table SELECT * FROM {$CFG->prefix}$table $earlycheck GROUP BY $groups HAVING COUNT(*) > 1",false)) {
        $dupes = get_records_sql("SELECT t.* FROM {$CFG->prefix}$table t, $temp_table d WHERE ($where) AND t.id <> d.id");
        execute_sql("DROP TABLE $temp_table", false);
...
class problem_000012 extends problem_base {
    function title() {
        return 'Orphaned Groups / Group Members Detected';
    }
    function exists() {
        global $CFG;
        $group_members = count_records_sql("SELECT COUNT(*) FROM {$CFG->prefix}groups_members WHERE userid NOT IN (SELECT id FROM {$CFG->prefix}user) OR groupid NOT IN (SELECT id FROM {$CFG->prefix}groups)");
        $bad_groups = count_records_sql("SELECT COUNT(*) FROM {$CFG->prefix}groups WHERE courseid NOT IN (SELECT id FROM {$CFG->prefix}course)");
        return ($group_members + $bad_groups > 0);
    }
    function severity() {
        return SEVERITY_SIGNIFICANT;
    }
    function description() {
        return 'There are entries in your database for groups or group members related to courses or users that do not exist.';
    }
    function solution() {
        global $CFG;
        if (optional_param('fixorphanedgroups', 0, PARAM_BOOL)) {
            execute_sql("DELETE FROM {$CFG->prefix}groups_members WHERE userid NOT IN (SELECT id FROM {$CFG->prefix}user)");
            execute_sql("DELETE FROM {$CFG->prefix}groups_members WHERE groupid NOT IN (SELECT id FROM {$CFG->prefix}groups)");
            execute_sql("DELETE FROM {$CFG->prefix}groups WHERE courseid NOT IN (SELECT id FROM {$CFG->prefix}course)");
            return 'Orphaned entries were removed.';
        } else {
            return '<a href="'.me().'&fixorphanedgroups=1">Delete the orphaned entries from the database.</a>';
        }
    }
}
...
class problem_000013 extends problem_base {
    function title() {
        return 'Orphaned Role Assignments Detected';
    }
    function exists() {
        global $CFG;
        $orphans_count = count_records_sql("SELECT COUNT(*) FROM {$CFG->prefix}role_assignments WHERE userid NOT IN (SELECT id FROM {$CFG->prefix}user)");
    ...
    function description() {
        return 'You have roles assigned to non-existent users. This could be caused by a malfunctioning enrolment plugin.';
    }
    function solution() {
        global $CFG;
        if (optional_param('fixorphanedroleassignments', 0, PARAM_BOOL)) {
            execute_sql("DELETE FROM {$CFG->prefix}role_assignments WHERE userid NOT IN (SELECT id FROM {$CFG->prefix}user)");
            return 'Orphaned entries were removed.';
        } else {
            return '<a href="'.me().'&fixorphanedroleassignments=1">Delete the orphaned entries from the database.</a>';
        }
    }
}
...
class problem_000014 extends problem_base {
    function get_dupes() {
        ...
        static $dupes = false;
        if (!is_array($dupes)) {
            $dupes = health_find_duplicates('grade_items', 'courseid', 'categoryid', 'itemtype', 'itemmodule', 'iteminstance', 'itemnumber');
            $dupes2 = health_find_duplicates('grade_items', 'courseid', 'itemtype="course"');
            if (is_array($dupes2)) {
                foreach($dupes2 as $id => $dupe) {
                    if (!isset($dupes[$id])) {
                        $dupes[$id] = $dupe;
                    }
                }
    ...
    function title() {
        return 'Duplicate entries in grade_items';
    }
    function exists() {
        return count($this->get_dupes()) > 0;
    }
    function severity() {
        return SEVERITY_SIGNIFICANT;
    }
    function description() {
        echo '<!-- @@ '; print_r($this->get_dupes()); echo ' -->';
        return 'You have '.count($this->get_dupes()).' duplicate entries in the grade_items table. This is a severe problem that may prevent the gradebook from working properly (or at all) in any course that is affected.'; 
    }
    function solution() {
        global $CFG;
        if (optional_param('fixgradebookduplicates', 0, PARAM_BOOL) && is_array($this->get_dupes()) && count($this->get_dupes()) > 0) {
            $ids = implode(',', array_keys($this->get_dupes()));
            if (execute_sql('DELETE FROM '.$CFG->prefix.'grade_items WHERE id IN ('.$ids.')')) {
                return 'The duplicates have been removed.';
            }
            return 'Unable to remove the duplicates.';
        } else {
            return 'Duplicate entries need to be deleted. <a href="'.me().'&fixgradebookduplicates=1">Do this automatically.</a>';
        }
    }
}
...
class problem_000015 extends problem_base {
    function title() {
        return 'Invalid course entries in mdl_grade_items';
    }
    function exists() {
        global $CFG;
        $broken = get_records_sql('SELECT * FROM '.$CFG->prefix.'grade_items WHERE itemtype="course" AND (iteminstance NOT IN (SELECT id FROM '.$CFG->prefix.'grade_categories) OR iteminstance IS NULL)');
        return is_array($broken) && count($broken) > 1;
    }
    function severity() {
        return SEVERITY_SIGNIFICANT;
    }
    function description() {
        return 'There are invalid course entries in the grade_items table. This will cause problems with the gradebook.';
    }
    function solution() {
        global $CFG;
        if (optional_param('fix15', 0, PARAM_BOOL)) {
            $broken = get_records_sql('SELECT * FROM '.$CFG->prefix.'grade_items WHERE itemtype="course" AND (iteminstance NOT IN (SELECT id FROM '.$CFG->prefix.'grade_categories) OR iteminstance IS NULL)');
            foreach ($broken as $id => $b) {
                $cat = get_record('grade_categories', 'courseid', $b->courseid, 'depth', 1);
                if (!$cat || $cat->id < 1) {
                    $cat = new object();
                    $cat->courseid=$b->courseid;
                    $cat->depth=1;
                    $cat->timecreated = time();
                    $cat->timemodified = time();
                    $cat->path = '/';
                    $cat->id = insert_record('grade_categories', $cat);
                    $cat->path = '/'.$cat->id;
                    update_record($cat);
                }
                execute_sql('UPDATE '.$CFG->prefix.'grade_items SET iteminstance='.$cat->id.' WHERE id='.$id);
            }
            return 'Fixed for you.';
        } else {
            return '<a href="'.me().'&fix15=1">Update the invalid entries, and add categories as necessary.</a>';
        }
    }
}
...
class problem_000016 extends problem_base {
    function title() {
        return 'Invalid categories in mdl_grade_items';
    }
    function exists() {
        global $CFG;
        $orphans_count = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'grade_items WHERE itemtype="category" AND iteminstance NOT IN (SELECT id FROM '.$CFG->prefix.'grade_categories)');
        return ($orphans_count > 0);
    }
    function severity() {
        return SEVERITY_SIGNIFICANT;
    ...
    function description() {
        return 'There are invalid entries in the grade_items table. This will cause problems with the gradebook.';
    }
    function solution() {
        global $CFG;
        if (optional_param('fixorphanedgradeitemscategories', 0, PARAM_BOOL)) {
            execute_sql('DELETE FROM '.$CFG->prefix.'grade_items WHERE itemtype="category" AND iteminstance NOT IN (SELECT id FROM '.$CFG->prefix.'grade_categories)');
            return 'Orphaned entries were removed.';
        } else {
            return '<a href="'.me().'&fixorphanedgradeitemscategories=1">Delete the orphaned entries from the database.</a>';
        }
    }
}
