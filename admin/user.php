<?php // $Id: user.php,v 1.109.2.4 2008/07/10 08:36:02 skodak Exp $
	...
	// nkowald - 2011-02-07 - Hacky hack because this page doesn't accept GET paramaters
	...
	$man_extrasql = '';
	if ($firstname != '') { 
		$man_extrasql = "firstname LIKE '%$firstname%'"; 
	}
	if ($lastname != '') {
		if ($firstname != '') {
			$man_extrasql .= " AND ";
		}
		$man_extrasql .= "lastname LIKE '%$lastname%'"; 
	}
	if ($idnumber != 0) {
		if ($lastname != '' || $firstname != '') {
			$man_extrasql .= " AND ";
		}
		$man_extrasql .= "idnumber LIKE '%$idnumber%'"; 
	}
	// nkowald
	...
	// nkowald - 2011-02-07 - Hacky, hacky
	if ($man_extrasql == '') {
		$extrasql = $ufiltering->get_sql_filter();
	} else {
		$extrasql = $man_extrasql;
	}
	...
	// nkowald - 2011-02-08 - Changing paging bar to keep my filters
	$base_page = $_SERVER['REQUEST_URI'];
    print_paging_bar($usercount, $page, $perpage, $base_page . "&amp;sort=$sort&amp;dir=$dir&amp;perpage=$perpage&amp;");
    //print_paging_bar($usercount, $page, $perpage,"user.php?sort=$sort&amp;dir=$dir&amp;perpage=$perpage&amp;");
    ...
    if (!empty($table)) {
        print_table($table);
		// nkowald - 2011-02-07 - get current page with params intact
		$base_page = 'user.php';
		//$base_page = $_SERVER['REQUEST_URI'];
		// nkowald - 2011-12-19 - If base_page contains a question mark then use ampersand first, otherwise use question mark
		$first_char = (strstr($base_page, '?') !== false) ? '&amp;' : '?' ;
		
		// If idnumber, firstname, lastname exists in query string: add them to pagination
		$added = '';
		$added .= (isset($_GET['idnumber']) && $_GET['idnumber'] != '' && is_numeric($_GET['idnumber'])) ? '&idnumber='. trim($_GET['idnumber']) : '';
		$added .= (isset($_GET['firstname']) && $_GET['firstname'] != '') ? '&firstname='. trim($_GET['firstname']) : '';
		$added .= (isset($_GET['lastname']) && $_GET['lastname'] != '') ? '&lastname='. trim($_GET['firstname']) : '';
		
        print_paging_bar($usercount, $page, $perpage, $base_page . $first_char . "sort=$sort&amp;dir=$dir&amp;perpage=$perpage" . $added . "&amp;");
        if (has_capability('moodle/user:create', $sitecontext)) {
            print_heading('<a href="'.$securewwwroot.'/user/editadvanced.php?id=-1">'.get_string('addnewuser').'</a>');
        }
    }
