<?php // $Id: messageselect.php,v 1.15.2.5 2008/02/20 02:32:03 moodler Exp $
	...
	// nkowald - 2010-11-18 - added attachement param
	$attach = optional_param('attachment','',PARAM_RAW);
	$del = optional_param('del', '', PARAM_RAW);
	$from_naked_form = optional_param('from_naked_form', '', PARAM_RAW);
...
// nkowald - 2010-11-18 - Added email attachment
	if ($attach != '') {
        $SESSION->attachment[$id] = $attach;
    } else {
		if ($from_naked_form == 'yes') {
			unset($SESSION->attachment);
		}
	}
    $attach = $SESSION->attachment[$id];
	
	// Remove attachment through get param
	if ($del != '' && $del = 'attachment') {
		unset($SESSION->attachment);
		$attach = '';
	}
...
';
                echo "<h3 style=\"font-weight:bold;\">".get_string('previewhtml')."</h3><br />
				<div class=\"messagepreview\">\n".format_text(stripslashes($messagebody),$format)."\n</div>\n";
				// nkowald - 2010-11-18 - Added attachment preview
	
				if (isset($attach) && $attach != '') {
					echo "<br />";
					echo "<div class=\"emailattachment\"><b>Attachment:</b> $attach\n";
				}
				// nkowald
                ...
                foreach ($SESSION->emailto[$id] as $user) {
					// nkowald - 2010-11-18 - Added filepath
					$filepath = '\\' . $id . '\\' . $attach;
                    $good = $good && message_post_message($USER,$user,addslashes($messagebody),$format,'direct',$filepath);
					if (isset($SESSION->attachment)) {
						unset($SESSION->attachment);
					}
                    if ($user->teacher) {
                        $teachers[] = $user->id;
