...
function useredit_shared_definition(&$mform) {
	...
	// Check what role current logged in user has, show or hide firstname/lastname based on role.
	$sql = "SELECT userid FROM mdl_role_assignments where roleid IN (1,17)";
	$is_admin = FALSE;
	if ($admins = get_records_sql($sql)) {
		foreach ($admins as $admin_id) {
			if ($USER->id == $admin_id->userid) {
				$is_admin = TRUE;
			}
		}
	}
	...
	$disabled_status = (!$is_admin) ? ' readonly="readonly"' : '';


	if ($is_admin) {
	
		if (fullname($nameordercheck) == 'b a' ) {  // See MDL-4325
			$mform->addElement('text', 'lastname',  get_string('lastname'),  'maxlength="100" size="30"'.$disabled_status.'');
			$mform->addElement('text', 'firstname', get_string('firstname'), 'maxlength="100" size="30"'.$disabled_status.'');
		} else {
			$mform->addElement('text', 'firstname', get_string('firstname'), 'maxlength="100" size="30"'.$disabled_status.'');
			$mform->addElement('text', 'lastname',  get_string('lastname'),  'maxlength="100" size="30"'.$disabled_status.'');
		}
		...
		// nkowald - 2010-09-07 - Made not required for now
    ...
    if (!empty($CFG->gdversion)) {
		// nkowald - 2011-05-04 - Removed ability to edit or delete default user picture
