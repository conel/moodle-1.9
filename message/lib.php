 ...
 */
function message_post_message($userfrom, $userto, $message, $format, $messagetype, $filepath='') {
...
/// Check to see if anything else needs to be done with it
    ...
    if ($emailforced || (!isset($preference->message_emailmessages) || $preference->message_emailmessages)) {  // Receiver wants mail forwarding
        ...
        if ($emailforced || (time() - $userto->lastaccess) > ((int)$preference->message_emailtimenosee * 60)) { // Long enough
			...
			// nkowald - 2010-11-18 - Add attachment, if added.
			if ($filepath != '' && file_exists($CFG->dataroot . $filepath)) {
				$filename_arr = explode('\\', $filepath);
				$filename = $filename_arr[(count($filename_arr) - 1)];

				if (email_to_user($userto, $userfrom, $messagesubject, $messagetext, $messagehtml, $filepath, $filename)) {
					$CFG->messagewasjustemailed = true;
				}
			} else {
				// nkowald - 2011-09-29 - Turning off for now.
