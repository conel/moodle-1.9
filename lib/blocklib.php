...
function blocks_print_group(&$page, &$pageblocks, $position) {
    ...
    // nkowald - 2010-07-15 - Slight change, should check against MOODLE_PAGE now I've added two new types
	$editmymoodle = $page->type == MOODLE_PAGE && has_capability('moodle/my:manageblocks', $coursecontext);
    ...
    switch($blockaction) {
        case 'config':
            ...
            if($data = data_submitted()) {
				...
				// nkowald - 2010-07-16 - Had to add this parameter so we could add and edit HTML blocks from the admin > stickyblocks page
				$pt = (!isset($pt) && isset($_GET['pt'])) ? $_GET['pt'] : '';

                print_heading($strheading);
				// nkowald - 2010-07-16
                echo '<form method="post" name="block-config" action="'. $page->url_get_path() .'?pt='.$pt.'">';
                echo '<p>';
...
function blocks_execute_repositioning(&$instance, $newpos, $newweight, $pinned=false) {
    ...
    if (!empty($pinned)) {
		/* nkowald - 2010-10-29
         * No idea why this is being used.
         * Pinned blocks are not stored in mdl_block_instance and so no weights should need shifting
         * Can't see any reason for this code to be ran
         *
