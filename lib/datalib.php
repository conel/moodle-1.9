...
function get_my_courses($userid, $sort='visible DESC,sortorder ASC', $fields=NULL, $doanything=false,$limit=0) {
    ...
    if ($userid === $USER->id) {
        ...
        } elseif (isset($USER->mycourses) && is_string($USER->mycourses)) {
            if ($USER->mycourses === '') {
	...
	*/
    ...
    for ($n=0; $n<$courses_count; $n++) {
		...
		//var_dump($cansee);
		// nkowald - 2011-10-14 - Forced this to be true as it was stopping courses a user had access to from showing, only 'enrolled' on courses showed
		$cansee = true;
        if ($cansee === true) {
 ...
 * nkowald - 2010-11-11
 *
 * Get ONLY courses a user is enrolled in.
 *
 * @param	$user_id	int		the user id we want to get course enrolments for
 * @return	array/boolean		returns courses array containing course objects if found OR FALSE if queries failed.
 *
 */
function get_my_enrolled_courses($user_id) {

	// Get context from user's role assignments, exclude blanket "1" assignement
	$query = "SELECT DISTINCT contextid FROM mdl_role_assignments WHERE userid = $user_id and contextid != 1";
	if ($contexts = get_records_sql($query)) {
		foreach ($contexts as $context) {
			$context_ids[] = $context->contextid;
		}
		$contexts_csv = implode(',', $context_ids);
		
		$query = "SELECT instanceid FROM mdl_context WHERE id IN ($contexts_csv) AND contextlevel = 50";
		if ($instance_ids = get_records_sql($query)) {
			$courses = array();
			foreach ($instance_ids as $course_id) {
				if ($course = get_record('course', 'id', $course_id->instanceid)) {
					$courses[$course->shortname] = $course;
				}
				// nkowald - 2011-09-13 - Sort courses by name
				ksort($courses);
			}
			// We now have an array of course objects, return them
			return $courses;
		} else {
			return false;
		}
...
// nkowald - 2011-08-19 - Output Course & Unit Progress if it exists
function get_unit_progress($user_id='') {

    global $CFG;
    $html = '<h2>Course & Unit Progress</h2>';

    // Get ids of courses this user is enrolled on
    if ($user_id != '') {
        if ($enrolled_courses = get_my_enrolled_courses($user_id)) {
            // Convert course_ids to a comma separated string
            foreach ($enrolled_courses as $course) {
                $course_ids[] = $course->id;
            }
            $courses_csv = implode(',', $course_ids); 
			// Get block id of assmgr block
			
			$blockid = 52; // default just in case
			if ($block = get_record('block', 'name', 'assmgr')) {
				$blockid = $block->id;
			}
            // query block instances seeing if any of these courses contain the assmgr block - needed to display data
            $query = "SELECT pageid FROM ".$CFG->prefix."block_instance WHERE pageid IN ($courses_csv) and blockid = $blockid";

            if ($course_ids = get_records_sql($query)) {
                foreach($course_ids as $id) {
                    $captured_cid = $id->pageid;
                }
                $html .='
                <script type="text/javascript">
                //<![CDATA[
                    function resizeIframe(newHeight) {
                        document.getElementById("outcomes").style.height = parseInt(newHeight) + 10 + "px";
                    }
                //]]>
                </script>
                <iframe src="'.$CFG->wwwroot.'/blocks/assmgr/actions/show_progress_ilp.php?course_id='.$captured_cid.'&candidate_id='.$user_id.'" width="100%" height="72" id="outcomes" frameBorder="0"></iframe>
                ';
                // Finally, return this code
                return $html;
            } else {
                $html .= '<p>Assessment manager block not added in any of this user\'s courses</p>';
            }
        } else {
            $html .= '<p>User not enrolled in any courses</p>';
        }
        return $html;
    } else {
        $html .= '<p>User required</p>';
        return $html;
    }
}
 ...
 */
function get_categories($parent='none', $sort=NULL, $shallow=true, $visibleonly=false) {
    global $CFG;
...
function fix_course_sortorder($categoryid=0, $n=0, $safe=0, $depth=0, $path='') {
    ...
    if ($categoryid > 0){
		...
		// nkowald - 2010-06-15 - Uncommented this as it's needed
		// get some basic info about courses in the category
		/*
		$query = 'SELECT MIN(sortorder) AS min,
