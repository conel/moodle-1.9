...
function error ($message, $link='') {
    ...
    if (empty($link) and !defined('ADMIN_EXT_HEADER_PRINTED')) {
        if ( !empty($SESSION->fromurl) ) {
			...
			// nkowald - 2010-07-07 - Redirecting to home is annoying, redirect to 'back' page instead.
            //$link = $CFG->wwwroot .'/';
			$link = '';
        }
    }
    ...
    if (!empty($link)) {
		...
		// Get URL of back button
		$back_link  = $_SERVER['HTTP_REFERER'];
		$back_link_url = ($back_link != '') ? $back_link : $link = $CFG->wwwroot .'/';
		print_continue($back_link_url);
	}
