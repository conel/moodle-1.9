    ...
    function regrade_final_grades($userid=null) {
        ...
        if ($rs) {
            while ($grade_record = rs_fetch_next_record($rs)) {
				...
				// nkowald - 2011-02-03 - This should hopefully fix initial setting bug.
				// It used to assign a finalgrade of 1 when first adding assignments, for some reason I can't work out.
				// It will not only update the final grade if a final grade is SET!
    ...
    */
    ...
    function update_final_grade($userid, $finalgrade=false, $source=NULL, $feedback=false, $feedbackformat=FORMAT_MOODLE, $usermodified=null) {
        ...
        if (empty($usermodified)) {
			// nkowald - 2011-06-21 - If usermodified is empty we need to make sure it uses the logged in as user
			// Get id of logged in as user
			/*
			if ($USER->username != '' && $found_user = get_record('user', 'username', $USER->username)) {
				$logged_in_as = $found_user->id;
			} else {
				// Set logged in as current user
				$logged_in_as = $USER->id;
			}
			$grade->usermodified = $logged_in_as;
			*/
