...
function question_has_capability_on($question, $cap, $cachecat = -1){
    ...
    if (array_search($cap, $question_questioncaps)!== FALSE){
        if (!has_capability('moodle/question:'.$cap.'all', get_context_instance_by_id($category->contextid))){
			// nkowald - 2011-08-03 - Question capability was returning false when the context could no longer be found, check here that this capability exists!
			if (!record_exists('context', 'id', $category->contextid)) {
				return true;
			}
