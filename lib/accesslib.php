...
function has_capability_in_accessdata($capability, $context, $accessdata, $doanything) {
	...
	switch($capability) {
		...
		case 'mod/questionnaire:view':
			// If Teacher role (/1:3 <-- 3 = teacher) has Mode > Questionnaire view role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:3']['mod/questionnaire:view']) && $USER->access['rdef']['/1:3']['mod/questionnaire:view'] == '1') {
				$can = 1;
			}
			// If non-editing teacher role (/1:4 <-- 4 = non-editing teacher) has Mode > Questionnaire view role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:4']['mod/questionnaire:view']) && $USER->access['rdef']['/1:4']['mod/questionnaire:view'] == '1') {
				$can = 1;
			}
			// If Student role (/1:5 <-- 5 = student) has Mode > Questionnaire view role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:5']['mod/questionnaire:view']) && $USER->access['rdef']['/1:5']['mod/questionnaire:view'] == '1') {
				$can = 1;
			}
			// If E-learning Technologist role (/1:2 <-- 3 = elt) has Mode > Questionnaire submit role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:2']['mod/questionnaire:view']) && $USER->access['rdef']['/1:2']['mod/questionnaire:view'] == '1') {
				$can = 1;
			}
			break;
			
		case 'mod/questionnaire:submit':
			// If Teacher role (/1:3 <-- 3 = teacher) has Mode > Questionnaire submit role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:3']['mod/questionnaire:submit']) && $USER->access['rdef']['/1:3']['mod/questionnaire:submit'] == '1') {
				$can = 1;
			} 		
			// If non-editing teacher role (/1:4 <-- 4 = non-editing teacher) has Mode > Questionnaire submit role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:4']['mod/questionnaire:submit']) && $USER->access['rdef']['/1:4']['mod/questionnaire:submit'] == '1') {
				$can = 1;
			}
			// If Student role (/1:5 <-- 5 = student) has Mode > Questionnaire submit role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:5']['mod/questionnaire:submit']) && $USER->access['rdef']['/1:5']['mod/questionnaire:submit'] == '1') {
				$can = 1;
			}
			// If E-learning Technologist role (/1:2 <-- 3 = elt) has Mode > Questionnaire submit role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:2']['mod/questionnaire:submit']) && $USER->access['rdef']['/1:2']['mod/questionnaire:submit'] == '1') {
				$can = 1;
			}
			break;
			
		case 'mod/questionnaire:createpublic':
			// If E-learning Technologist role (/1:2 <-- 3 = elt) has Mode > Questionnaire submit role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:2']['mod/questionnaire:createpublic']) && $USER->access['rdef']['/1:2']['mod/questionnaire:createpublic'] == '1') {
				$can = 1;
			}
			break;
		
		case 'mod/questionnaire:editquestions':
			// If E-learning Technologist role (/1:2 <-- 3 = elt) has Mode > Questionnaire submit role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:2']['mod/questionnaire:editquestions']) && $USER->access['rdef']['/1:2']['mod/questionnaire:editquestions'] == '1') {
				$can = 1;
			}
			break;
		
		case 'mod/questionnaire:copysurveys':
			// If E-learning Technologist role (/1:2 <-- 3 = elt) has Mode > Questionnaire submit role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:2']['mod/questionnaire:copysurveys']) && $USER->access['rdef']['/1:2']['mod/questionnaire:copysurveys'] == '1') {
				$can = 1;
			}
			break;
		
		case 'mod/questionnaire:manage':
			// If E-learning Technologist role (/1:2 <-- 3 = elt) has Mode > Questionnaire submit role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:2']['mod/questionnaire:manage']) && $USER->access['rdef']['/1:2']['mod/questionnaire:manage'] == '1') {
				$can = 1;
			}
			break;
		
		case 'mod/questionnaire:createtemplates':
			// If E-learning Technologist role (/1:2 <-- 3 = elt) has Mode > Questionnaire submit role enabled, then they need to view questionnare
			if (isset($USER->access['rdef']['/1:2']['mod/questionnaire:createtemplates']) && $USER->access['rdef']['/1:2']['mod/questionnaire:createtemplates'] == '1') {
				$can = 1;
			}
			break;
		
		case 'mod/forum:replypost':
		// If Student role (/1:5 <-- 5 = student)
			if (isset($USER->access['rdef']['/1:5']['mod/forum:replypost']) && $USER->access['rdef']['/1:5']['mod/forum:replypost'] == '1') {
				$can = 1;
			}
			break;
		
		case 'mod/forum:replynews':
			// If Student role (/1:5 <-- 5 = student)
			if (isset($USER->access['rdef']['/1:5']['mod/forum:replynews']) && $USER->access['rdef']['/1:5']['mod/forum:replynews'] == '1') {
				$can = 1;
			}
			break;
		...
		case 'moodle/user:editownprofile':
			...
			// Manager role
			if (isset($USER->access['rdef']['/1:2']['moodle/user:editownprofile']) && $USER->access['rdef']['/1:2']['moodle/user:editownprofile'] == '1') {
				$can = 1;
			}
		break;

		case 'moodle/user:update':
			// If Teacher role
			if (isset($USER->access['rdef']['/1:3']['moodle/user:update']) && $USER->access['rdef']['/1:3']['moodle/user:update'] == '1') {
				$can = 1;
			}
			// Manager role
			if (isset($USER->access['rdef']['/1:2']['moodle/user:update']) && $USER->access['rdef']['/1:2']['moodle/user:update'] == '1') {
				$can = 1;
			}
		break;
		
		// nkowald - 2010-10-15 - Hopefully this will fix staff being unassigned on meta courses.
		case 'moodle/course:managemetacourse':

			// Admin
			if (isset($USER->access['rdef']['/1:1']['moodle/course:managemetacourse']) && $USER->access['rdef']['/1:1']['moodle/course:managemetacourse'] == '1') {
				$can = 1;
			}
			// Manager
			if (isset($USER->access['rdef']['/1:2']['moodle/course:managemetacourse']) && $USER->access['rdef']['/1:2']['moodle/course:managemetacourse'] == '1') {
				$can = 1;
			}
			// Teacher
			if (isset($USER->access['rdef']['/1:3']['moodle/course:managemetacourse']) && $USER->access['rdef']['/1:3']['moodle/course:managemetacourse'] == '1') {
				$can = 1;
			}
			// E-learning Technologist
			if (isset($USER->access['rdef']['/1:17']['moodle/course:managemetacourse']) && $USER->access['rdef']['/1:17']['moodle/course:managemetacourse'] == '1') {
				$can = 1;
			}
			// Lead Personal Tutor
			if (isset($USER->access['rdef']['/1:15']['moodle/course:managemetacourse']) && $USER->access['rdef']['/1:15']['moodle/course:managemetacourse'] == '1') {
				$can = 1;
			}
		break;
		
		// nkowald - 2011-03-10 - Force view site stats assignment on these roles
		case 'mod/data:viewsitestats':
		
			// Admin
			if (isset($USER->access['rdef']['/1:1']['mod/data:viewsitestats']) && $USER->access['rdef']['/1:1']['mod/data:viewsitestats'] == '1') {
				$can = 1;
			}	
			// Manager
			if (isset($USER->access['rdef']['/1:2']['mod/data:viewsitestats']) && $USER->access['rdef']['/1:2']['mod/data:viewsitestats'] == '1') {
				$can = 1;
			}
			// E-learning Technologist
			if (isset($USER->access['rdef']['/1:17']['mod/data:viewsitestats']) && $USER->access['rdef']['/1:17']['mod/data:viewsitestats'] == '1') {
				$can = 1;
			}
...
/// OK, if they still don't have the capability then print a nice error message
    if (!has_capability($capability, $context, $userid, $doanything)) {
        // This is a bit shit, on IIS print_error shows 404 - still, better than allowing access
		//print_error($errormessage, $stringfile, $errorlink, $capabilityname);
		// nkowald - 2010-03-19 - Was not working properly so added this return false
		return false;
    } else {
		return true;
	}
...
function get_assignable_roles($context, $field='name', $rolenamedisplay=ROLENAME_ALIAS) {
	...
	$query = "SELECT ro.* FROM {$CFG->prefix}role ro,
                                          (
...
function get_assignable_roles_for_switchrole($context, $field='name', $rolenamedisplay=ROLENAME_ALIAS) {
	...
	// nkowald - 2011-10-03 - Need to remove e-learning technologists from this list, gives teachers access to things they shouldn't
	unset($roles[17]);
...
// nkowald - 2010-06-28 
// Function to get role of user for my moodle redirection
// role will return either staff or student.

function get_role_staff_or_student($user_id) {

	$role = 5; // user is student by default;
	if ($user_id != '' && is_numeric($user_id)) {
	
		// nkowald - updated to check for email first
		if ($user = get_record('user', 'id', $user_id)) {
			$email = $user->email;
			if (strpos($email, '@student.conel.ac.uk') !== false) {
				$role = 5;
			} else {
				// Staff members must use an email address in the format of *@staff.conel.ac.uk or *@conel.ac.uk
				if ((strpos($email, '@staff.conel.ac.uk') !== false) || (strpos($email, '@conel.ac.uk') !== false)) {
					$role = 3;
				}
			}
			return $role;
		}
		/* nkowald - 2011-11-08 - No longer used: works by checking the email address of the logged in user
		else {
			$query = sprintf("SELECT DISTINCT roleid FROM ".$CFG->prefix."role_assignments WHERE userid = %d", $user_id);
			...
			if ($roles = get_records_sql($query)) {
				...
				// This methodology is perhaps specific to our (CONEL's) Moodle install
				
				if ($num_roles > 1) {
						// if admin role found, set user as teacher
						if (in_array(1, $all_roles)) {
							$role = 3;
						}
						// if Manager role found, set user as teacher
						else if (in_array(17, $all_roles)) {
							$role = 3;
						}
						// if Manager role found, set user as teacher
						else if (in_array(15, $all_roles)) {
							$role = 3;
						}
						// if e-learning technologist role found, set user as teacher
						else if (in_array(2, $all_roles)) {
							$role = 3;
						}
						// if teacher role found, user is teacher
						else if (in_array(3, $all_roles)) {
							$role = 3;
						}
