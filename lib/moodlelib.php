    ...
    foreach ($auths as $auth) {
        ...
        if ($user->id) {                          // User already exists in database
            ...
            if (empty($user->firstaccess)) { //prevent firstaccess from remaining 0 for manual account that never required confirmation
				// nkowald - 2010-12-20 - Sometimes timemodified is 0 too, so first try to use lastlogin, then if blank, timemodified
				if ($user->lastlogin != 0) {
					set_field('user','firstaccess', $user->lastlogin, 'id', $user->id);
					$user->firstaccess = $user->lastlogin;
				} else if ($user->timemodified != 0) {
...
/// Delete course blocks
	...
	$query = "SELECT * FROM {$CFG->prefix}block_instance
                    WHERE pagetype = '".PAGE_COURSE_VIEW."'
                    AND pageid = $course->id";
	
    if ($blocks = get_records_sql($query)) {
