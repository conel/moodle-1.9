<?php // $Id: file.php,v 1.46.2.5 2009/04/09 09:30:32 skodak Exp $
    ...
    if (($args[0]!='blog') and ($args[0]!='user') and (!$course = get_record_sql("SELECT * FROM {$CFG->prefix}course WHERE id='".(int)$args[0]."'"))) {
        error('Invalid course ID');
    }

    // security: prevent access to "000" or "1 something" directories
    // hack for blogs, needs proper security check too
    if (($args[0] != 'blog') and ($args[0]!='user') and ($args[0] != $course->id)) {
        error('Invalid course ID');
    }
    ...
    if ($args[0] == 'blog') {
        ...
        // security: assessors can view all files submitted to their courses, students only their own

        if(!isset($args[3]) || $args[3]!='block_assmgr') {
        ...
        // get the evidence and all its submissions
        if($type == 'evidence') {
            // get the evidence and all its submissions
            $sql = "SELECT evid.candidate_id, evid.creator_id, port.course_id
                    FROM {$CFG->prefix}block_assmgr_evidence AS evid
                       LEFT JOIN {$CFG->prefix}block_assmgr_submission AS sub ON (sub.evidence_id = evid.id)
                       LEFT JOIN {$CFG->prefix}block_assmgr_portfolio AS port ON (sub.portfolio_id = port.id)
                    WHERE evid.id = {$record_id}";

        } else {
            $sql = "SELECT evid.candidate_id, evid.creator_id, port.course_id
                    FROM {$CFG->prefix}block_assmgr_evidence AS evid,
                         {$CFG->prefix}block_assmgr_submission AS sub,
                         {$CFG->prefix}block_assmgr_portfolio AS port
                    WHERE sub.id = {$record_id}
        ...
        if (current($submissions)->candidate_id == $USER->id
            || ($type == 'evidence' && current($submissions)->creator_id == $USER->id)) {
            // can view own files
        } else {
            $access_isassessor = false;
            foreach($submissions as $sub) {
                if(is_null($sub->course_id)) {
                    // is the user an admin
                    $access_isassessor = has_capability(
                        'moodle/site:doanything',
                        get_context_instance(CONTEXT_SYSTEM)
                    );
                } else {
                    // is the user an assessor in the submitted course
                    $access_isassessor = has_capability(
                        'block/assmgr:assessportfolio',
                        get_context_instance(CONTEXT_COURSE, $sub->course_id)
                    );
                }
