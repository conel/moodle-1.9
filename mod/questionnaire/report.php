<?php  // $Id: report.php,v 1.14.2.12 2008/12/05 17:18:25 joseph_rezeau Exp $
...
/// Tab setup:
    ...
    case 'dcsv': // download as text (cvs) format
		...
		if ($groupmode == 1 && $groupscount === 0 && !$user) {
			...
			// nkowald - 2010-03-15 - if a certain questionnaire add extra student details
			if ($questionnaire->id == 160) {
				// get student details
				...
				foreach ($output as $row) {
					...
					if ($i == 0) {
						...
						// add user details to row
						$rows[0] = implode("\t", $row);

					} else {

						// if not first item, is user record
						// get user details by id
						$response_id = $row[0];
						$user_details = array();
						$user_details = $valid->validate_get_user_details($response_id);
						$string_values = $valid->validate_get_user_details_as_csv_values($user_details);

						if ($i == 1) {

							// add key values to column titles
							$titles = '';
							$num_keys = count($string_values);
							$c = 1; // counter
							foreach ($string_values as $key => $value) {
								if ($c != $num_keys) {
									$titles .= $key . "\t";
								} else {
									$titles .= $key;
							...
							$rows[0] .= "\t" . $titles;
							$more_text = implode("\t", $string_values);
    ...
    /// Print the page header
        ...
        // nkowald - 2010-04-06 - get user details and display below answers
        echo '<br/><br/>';
        // JavaScript to slide down user details
		// nkowald - 2010-03-15 - only show if 'Help us to help you into University Survey' Survey

		if ($questionnaire->id == 160) {
			echo '<script>';
			echo '
				jQuery(document).ready(function(){
					// show/hide details based on cookie
					var show_it = readCookie("show_user_details");
					if (show_it) {
						if (show_it == "hide") {
							jQuery("#show_details").html("Show user details &gt;")
							jQuery("#user_details").attr("class","hide");
						} else {
							jQuery("#show_details").html("Hide user details &gt;")
							jQuery("#user_details").attr("class","show");
						}
					} else {
						createCookie("show_user_details","hide","60");
					}
					
					jQuery("#show_details").click(function(event){
						event.preventDefault();
						eraseCookie("show_user_details");

						// change link title
						if (jQuery("#user_details").is(":visible")) {
							jQuery("#show_details").html("Show user details &gt;")
							jQuery("#user_details").slideToggle();
							createCookie("show_user_details","hide","60");
						} else {
							jQuery("#show_details").html("Hide user details &gt;")
							jQuery("#user_details").slideToggle();
							createCookie("show_user_details","show","60");
						}
					});
				 });
			';
			echo '</script>';
			echo '<p><a href="#" id="show_details">Show user details &gt;</a></p>';
			echo '<div id="user_details">';
			echo '<h1>User Details</h1>';

			$valid = new validate_user();
			$user_details = $valid->validate_get_user_details($rid);
			if (count($user_details > 0)) {
				$valid->validate_print_user_details($user_details);
			}
			echo '<br /></div>';
