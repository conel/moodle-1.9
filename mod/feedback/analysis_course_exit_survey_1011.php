<?php // $Id: analysis_course.php,v 1.5.2.3 2008/07/18 14:54:43 agrabs Exp $
	...
	***/
	...
	if (isset($_GET['reset']) && $_GET['reset'] == 1 && (isset($_SERVER['HTTP_REFERER']) && !(strpos($_SERVER['HTTP_REFERER'], '&reset=1'))) ) {
		unset($_SESSION['ces1011']);
	} else {
		// nkowald - 24-08-2010 - If coming from 'Show Responses' tab, fill last used filters
		if (isset($_POST) && count($_POST) > 0) {
			$_SESSION['ces1011']['filters_post'] = $_POST;
		}
		if ($_SESSION['ces1011']['filters_post'] != '') {
			foreach ($_SESSION['ces1011']['filters_post'] as $key => $value) {
				$_POST[$key] = $value;
			}
		}
		// nkowald - 24-08-2010
		...
		*/
		...
		if ($courseitemfilter > 0) {
			...
			if ($search_directorates > 0 && $filter_choice == 'filter_directorate') {
				...
				$get_cats = "SELECT DISTINCT c.id, c.shortname, c.category, cia.year_1011
								...
								AND (c.shortname LIKE '%$searchcourse%' OR c.fullname LIKE '%$searchcourse%' OR cia.year_1011 LIKE '%$searchcourse%')
			...
			$sql = 'SELECT DISTINCT c.id, cia.year_1011 FROM 
						'.$CFG->prefix.'course c, '. $CFG->prefix.'feedback_value fv, '.$CFG->prefix.'feedback_item fi, '. $CFG->prefix.'course_idnumber_archive cia 
						  WHERE c.id = fv.course_id 
						  AND fi.id = fv.item 
						  AND fi.feedback = '.$feedback->id.' 
						  AND c.id = cia.course_id 
						  AND (c.shortname '.sql_ilike().' \'%'.$searchcourse.'%\' OR c.fullname '.sql_ilike().' \'%'.$searchcourse.'%\' OR cia.year_1011 LIKE \'%'.$searchcourse.'%\')
			...
			if ($filter_choice == "" || $filter_choice == "filter_course") {
				...
				$sql = "SELECT DISTINCT c.id, cia.year_1011
					FROM ".$CFG->prefix."course c, ".$CFG->prefix."feedback_value fv, ".$CFG->prefix."feedback_item fi, ".$CFG->prefix."course_idnumber_archive cia
					WHERE c.id = fv.course_id
					AND fi.id = fv.item
					AND c.id = cia.course_id
			...
			/*
            if (isset($_SESSION['ces1011']['where_clause']) && $where_clause == '') {
                $where_clause = $_SESSION['ces1011']['where_clause'];
            } else if ($where_clause != '') {
                $_SESSION['ces1011']['where_clause'] = $where_clause;
            }
			*/
            // nkowald - 24-08-2010
