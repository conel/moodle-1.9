<?php // $Id: analysis_course.php,v 1.5.2.3 2008/07/18 14:54:43 agrabs Exp $
	...
	***/
    ...
    if (isset($_POST) && count($_POST) > 0) {
        $_SESSION['cls']['filters_post'] = $_POST;
    }
    if ($_SESSION['cls']['filters_post'] != '') {
        foreach ($_SESSION['cls']['filters_post'] as $key => $value) {
            $_POST[$key] = $value;
        }
    }
    // nkowald - 24-08-2010
		...
		if ($courseitemfilter > 0) {
			...
			if ($search_directorates > 0 && $filter_choice == 'filter_directorate') {
				...
				$get_cats = 'SELECT DISTINCT c.id, c.shortname, c.category FROM '.$CFG->prefix.'course c, '.
												  ...
												  OR c.fullname '.sql_ilike().' \'%'.$searchcourse.'%\') ORDER BY c.shortname';
			...
			if ($search_schools > 0 && $filter_choice == 'filter_school') {
				...
				// convert filtered courses array to csv string
				$schools_found = implode(",", $school_courses);
				
				// If we've found courses under the schools category, lets display them.
				$sql = 'SELECT DISTINCT c.id, c.shortname FROM '.$CFG->prefix.'course c, '.
								  $CFG->prefix.'feedback_value fv, '.$CFG->prefix.'feedback_item fi '.
								  'WHERE c.id = fv.course_id 
								  AND fi.id = fv.item 
			...
			echo '</div>';
				...
				$sql = 'SELECT DISTINCT c.id, c.shortname FROM '.$CFG->prefix.'course c, '.
												  ...
												  AND (c.shortname LIKE \'__'.$search_levels.'%\') ORDER BY c.shortname';
			...
			*/
            ...
            if (isset($_SESSION['cls']['where_clause']) && $where_clause == '') {
                $where_clause = $_SESSION['cls']['where_clause'];
            } else if ($where_clause != '') {
                $_SESSION['cls']['where_clause'] = $where_clause;
            }
            // nkowald - 24-08-2010
