<?php // $Id: analysis_course.php,v 1.5.2.3 2008/07/18 14:54:43 agrabs Exp $
	...
	***/
	...
	if (isset($_GET['reset']) && $_GET['reset'] == 1 && (isset($_SERVER['HTTP_REFERER']) && !(strpos($_SERVER['HTTP_REFERER'], '&reset=1'))) ) {
		unset($_SESSION['cls1112']);
	} else {
		// nkowald - 24-08-2010 - If coming from 'Show Responses' tab, fill last used filters
		if (isset($_POST) && count($_POST) > 0) {
			$_SESSION['cls1112']['filters_post'] = $_POST;
		}
		if ($_SESSION['cls1112']['filters_post'] != '') {
			foreach ($_SESSION['cls1112']['filters_post'] as $key => $value) {
				$_POST[$key] = $value;
			}
		}
		// nkowald - 24-08-2010
	...
	if (isset($_GET['noresponses']) && $_GET['noresponses'] == 1) {
		...
		if ($unanswered_results = get_records_sql($sql)) {
			...
			foreach($unanswered_results as $result) {
				if ($result->id != 1) {
					...
					$no_results[] = '<a href="'.$CFG->wwwroot.'/course/view.php?id='.$result->id.'">'.$id_is.' - '.$result->fullname.'</a>';
				}
			}
		}
		...
		//show the count
		if(is_array($items)){
			echo '<a href="analysis_course.php?id=' . $id . '&amp;courseid=0&amp;reset=1">'.get_string('show_all', 'feedback').' courses</a>';
		} else {
			$items=array();
		}
		...
		*/
		...
		if ($courseitemfilter > 0) {
			...
			if ($search_directorates > 0 && $filter_choice == 'filter_directorate') {
				...
				$get_cats = "SELECT DISTINCT c.id, c.shortname, c.category, cia.year_1112
								...
								AND (c.shortname LIKE '%$searchcourse%' OR c.fullname LIKE '%$searchcourse%' OR cia.year_1112 LIKE '%$searchcourse%')
			...
			$sql = 'SELECT DISTINCT c.id, cia.year_1112 FROM 
						'.$CFG->prefix.'course c, '. $CFG->prefix.'feedback_value fv, '.$CFG->prefix.'feedback_item fi, '. $CFG->prefix.'course_idnumber_archive cia 
						  WHERE c.id = fv.course_id 
						  AND fi.id = fv.item 
						  AND fi.feedback = '.$feedback->id.' 
						  AND c.id = cia.course_id 
						  AND (c.shortname '.sql_ilike().' \'%'.$searchcourse.'%\' OR c.fullname '.sql_ilike().' \'%'.$searchcourse.'%\' OR cia.year_1112 LIKE \'%'.$searchcourse.'%\')
			...
			//$valid_curric_ids = "385, 122, 399, 45, 39, 22, 58, 124, 24, 61, 40, 3, 306, 4, 205, 42, 117, 118, 119, 123, 400, 386, 46, 63, 59, 41, 50, 333, 49, 398, 120, 121, 43, 401, 29, 402, 335, 125, 263, 30, 51, 334, 31, 393, 16, 33, 44, 394";
			// nkowald - 2011-05-20 - Updated curriculum areas to match stats
			$valid_curric_ids = "385, 122, 399, 45, 39, 22, 562, 124, 24, 61, 40, 3, 306, 4, 205, 42, 117, 118, 119, 123, 400, 386, 46, 63, 59, 41, 50, 333, 49, 398, 120, 121, 43, 401, 29, 402, 335, 125, 263, 30, 51, 334, 31, 393, 16, 33, 44, 394, 47, 48";
			...
			if ($filter_choice == "" || $filter_choice == "filter_course") {
				...
				$sql = "SELECT DISTINCT c.id, cia.year_1112
					FROM ".$CFG->prefix."course c, ".$CFG->prefix."feedback_value fv, ".$CFG->prefix."feedback_item fi, ".$CFG->prefix."course_idnumber_archive cia
					WHERE c.id = fv.course_id
					AND fi.id = fv.item
					AND c.id = cia.course_id
			...
			$filter_site_both = (isset($filter_site) && ($filter_site == '')) ? ' selected="selected"' : '';
			$filter_site_tottenham = (isset($filter_site) && ($filter_site == '1')) ? ' selected="selected"' : '';
			$filter_site_enfield = (isset($filter_site) && ($filter_site == '2')) ? ' selected="selected"' : '';
			...
			$query = "SELECT id FROM ".$CFG->prefix."feedback_item WHERE feedback = ".$feedback->id." AND name = 'Which site do you study at?'";
			$id_of_site = get_records_sql($query);
			foreach ($id_of_site as $sid) {
				$site_id = $sid->id;
			}
			...
			if (($site != '' && $site_id != '')) {
				// Site ONLY
				$query = "SELECT completed FROM ".$CFG->prefix."feedback_value WHERE item = ".$site_id." and value = ".$site."";
				$users_by_site = get_records_sql($query);		
				foreach ($users_by_site as $user) {
					$ubs[] = $user->completed;
				}	
			...
			if ($site != '' || $gender != '' || $age != '' || $ethnic != '' || $attend != '' || $dld != '') {
			...
			/*
            if (isset($_SESSION['cls1112']['where_clause']) && $where_clause == '') {
                $where_clause = $_SESSION['cls1112']['where_clause'];
            } else if ($where_clause != '') {
                $_SESSION['cls1112']['where_clause'] = $where_clause;
            }
			*/
            // nkowald - 24-08-2010
