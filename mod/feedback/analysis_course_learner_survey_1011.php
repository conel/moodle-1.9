<?php // $Id: analysis_course.php,v 1.5.2.3 2008/07/18 14:54:43 agrabs Exp $
	...
	***/
	...
	if (isset($_GET['reset']) && $_GET['reset'] == 1 && (isset($_SERVER['HTTP_REFERER']) && !(strpos($_SERVER['HTTP_REFERER'], '&reset=1'))) ) {
		unset($_SESSION['cls1011']);
	} else {
		// nkowald - 24-08-2010 - If coming from 'Show Responses' tab, fill last used filters
		if (isset($_POST) && count($_POST) > 0) {
			$_SESSION['cls1011']['filters_post'] = $_POST;
		}
		if ($_SESSION['cls1011']['filters_post'] != '') {
			foreach ($_SESSION['cls1011']['filters_post'] as $key => $value) {
				$_POST[$key] = $value;
			}
		}
		// nkowald - 24-08-2010
		...
		if ($courseitemfilter > 0) {
			...
			if ($search_levels != '0' && $filter_choice == 'filter_level') {
				...
				if ($courses = get_records_sql($sql)) {
					...
					// nkowald - 2010-10-12 - Need to show 1011 academic year idnumbers instead
			...
			/*
            if (isset($_SESSION['cls1011']['where_clause']) && $where_clause == '') {
                $where_clause = $_SESSION['cls1011']['where_clause'];
            } else if ($where_clause != '') {
                $_SESSION['cls1011']['where_clause'] = $where_clause;
            }
			*/
            // nkowald - 24-08-2010
