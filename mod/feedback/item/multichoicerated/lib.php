...
class feedback_item_multichoicerated extends feedback_item_base {
    ...
    function get_analysed($item, $groupid = false, $courseid = false, $where_clause = false) {
		...
		// nkowald - 2009-12-07 - My attempt to get values to return when excel printing
        ...
        for($i = 1; $i <= count($lines); $i++) {
            ...
            $ans = null;
			// nkowald - 2009-12-07 - Trimmed Answer text as it was messing up the excel print
            $ans->answertext = trim($item_values[1]);
            $avg = 0.0;
            $anscount = 0;
            ...
            $ans->answer_count = $anscount;
            $ans->avg = doubleval($avg) / doubleval(count($values));
            $ans->value = $item_values[0];
            $ans->quotient = $ans->answer_count / count($values);
            $analysedAnswer[] = $ans;
        }
    ...
    function print_analysed($item, $itemnr = '', $groupid = false, $courseid = false, $where_clause = false) {
        ...
        $analysedItem = $this->get_analysed($item, $groupid, $courseid, $where_clause);
        if($analysedItem) {
            //echo '<table>';
            // $itemnr++;
            ...
            foreach($analysedVals as $val) {
                ...
                echo '<tr><td align="left" valign="top">-&nbsp;&nbsp;' . $val->answertext . ' ('.$val->value.'):</td><td align="left" style="width: '.FEEDBACK_MAX_PIX_LENGTH.'"><img alt="'.$intvalue.'" src="'.$pix.'" height="5" width="'.$pixwidth.'" />' . $val->answer_count. (($val->quotient > 0)?'&nbsp;('. $quotient . '&nbsp;%)':'') . '</td></tr>';
            }
    ...
    function excelprint_item(&$worksheet, $rowOffset, $item, $groupid, $courseid = false, $where_clause = false) {
        ...
        if(is_array($data)) {
            ...
            for($i = 0; $i < count($data); $i++) {
                ...
                $worksheet->write_string($rowOffset, $i + 1, $aData->answertext);
                
                $worksheet->setFormat("<l><vo>");
                $worksheet->write_number($rowOffset + 1, $i + 1, $aData->answer_count);
            ...
            $worksheet->write_string($rowOffset, count($data) + 1, get_string('average', 'feedback'));
            
            $worksheet->setFormat("<l><f><vo>");
            $worksheet->write_number($rowOffset + 1, count($data) + 1, $avg);
        }
        $rowOffset +=2 ;
        return $rowOffset;
    }
