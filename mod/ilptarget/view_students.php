...
/// and if so, set $currentgroup to reflect the current group
	...
	if (($ausers = get_records_sql($select.$from.$where.$wheresearch.$sort, $table->get_page_start(), $table->get_page_size())) !== false) {
            foreach ($ausers as $auser) {
				...
				$targettotal = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'ilptarget_posts WHERE setforuserid = '.$auser->id.' AND status != 3 AND timecreated >= '.$ts_year_start.'' );
				//$targettotal = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'ilptarget_posts WHERE setforuserid = '.$auser->id.' AND status != 3' );
				$targetcomplete = count_records_sql('SELECT COUNT(*) FROM '.$CFG->prefix.'ilptarget_posts WHERE setforuserid = '.$auser->id.' AND status = 1 AND timecreated >= '.$ts_year_start.'');
		...
		print_header("$strtargets", "", "$navigation -> ".get_string('mystudents', 'targets')."", "", "", true, "", "");
