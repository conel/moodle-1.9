...
class ilptarget_updatetarget_form extends moodleform {
    ...
    function definition() {
		...
		// nkowald - 2011-08-08 - Adding a new hidden field to set whether coming from form or not
        $mform->addElement('hidden', 'saved', 1);
        ...
        $mform->addElement('htmleditor', 'targetset', 'Set S.M.A.R.T. Target<br />(Specific, Measurable, Attainable, Relevant, Time-bound)');
    ...
    function process_data($data) {
		...
		// nkowald - 2010-06-05 - Fixed link error
...
function ilptarget_update_status_menu($targetpost,$context) {
	...
	// nkowald - 2010-06-22 - Don't want this editing time stuff
	/*
	...
	*/
	...
	if((($ownpost) || has_capability('moodle/site:doanything', $context)) || has_capability('mod/ilptarget:addcomment', $context)) {
			$output .= ' | <a title="'.get_string('edit').'" href="'.$CFG->wwwroot.'/mod/ilptarget/target_view.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'userid='.$user->id.'&amp;targetpost='.$report->id.'&amp;action=updatetarget"><img src="'.$CFG->pixpath.'/t/edit.gif" alt="'.get_string('edit').'" /> '.get_string('edit').'</a> | <a title="'.get_string('delete').'" href="'.$CFG->wwwroot.'/mod/ilptarget/target_view.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'userid='.$user->id.'&amp;targetpost='.$report->id.'&amp;action=delete""><img src="'.$CFG->pixpath.'/t/delete.gif" alt="'.get_string('delete').'" /> '.get_string('delete').'</a> | ';
	}
	
	// nkowald - 2010-10-14 - Added 'has_capability('mod/ilptarget:updatetargetstatus', $context)' to check as ELTs need to be able to update these
	if($ownpost || ($tutorpost && has_capability('mod/ilptarget:viewclass', $context)) || has_capability('moodle/site:doanything', $context) || has_capability('mod/ilptarget:updatetargetstatus', $context)){
...
function ilptarget_update_comment_menu($commentid,$context) {
	...
	*/
	...
	// nkowald - 2010-06-22 - Admins, E-Learning Technologists and Teachers need to be able to edit comments
	/*
	if ($roles = get_user_roles($context, $USER->id)) {
		$users_roles = array();
		foreach ($roles as $role) {
			if ($role->roleid > 0) {
				$users_roles[] = intval($role->roleid);
			}
		}
		$users_roles = array_unique($users_roles);
	}
	
	$can_edit_comments = array(1,2,3);
	$can_edit = FALSE;
	foreach($users_roles as $their_role) {
		if (in_array($their_role, $can_edit_comments)) {
			$can_edit = TRUE;
		}
	}
	*/
