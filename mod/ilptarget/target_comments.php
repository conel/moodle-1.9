 ...
 */
	...
	$this_page = ($_SERVER['REQUEST_URI'] != '') ? $_SERVER['REQUEST_URI'] : 'target_comments.php';
    add_to_log($courseid, "ilp", "view target comments", $this_page, $targetpost);
	...
	if($USER->id != $user->id){
		...
		$mform = new ilptarget_updatecomment_form('', array('userid' => $user->id, 'id' => $id, 'courseid' => $courseid, 'targetpost' => $targetpost, 'commentid' => $commentid));
				...
				echo '</div>';
					...
					if(($post->status == 0 || has_capability('moodle/site:doanything', $context)) || has_capability('mod/ilptarget:addcomment', $context)){
