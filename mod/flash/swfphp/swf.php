<?php
 ...
 // +----------------------------------------------------------------------+
 // | More info on swf file format at :                                                              |
 ...
 // +----------------------------------------------------------------------+
 // | Authors: Original Author <alessandro@sephiroth.com>                  |
 // +----------------------------------------------------------------------+
  /**
 ...
 class SWF
 {
    ...
    /**
     ...
     function setMovieSize($width, $height)
     {
        ...
        $wtwips=$width*20; //width and height in 'twips'
        $htwips=$height*20;
        //always use nbits of 15 - this is usual in header according to MM file format doc 
        $nbits='01111';
        $xmin='000000000000000';
        $ymin='000000000000000';
        $ymax=str_pad(decbin($htwips), 15, "0", STR_PAD_LEFT);  
        $xmax=str_pad(decbin($wtwips), 15, "0", STR_PAD_LEFT);
        $rect=$nbits. $xmin. $xmax. $ymin. $ymax. '0000000'; //pad to byte boundary
        $rectdata='';
        for ($byte=0; $byte<9; $byte++)
        {
            $bytedec=bindec(substr($rect, $byte*8, 8));
            $rectdata.=pack('C', $bytedec);
        }
        $this->data = substr($this->data, 0, 8) . $rectdata . substr($this->data, 17);
    }     
     /**
