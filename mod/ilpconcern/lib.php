...
function update_student_status_menu($userid,$courseid) {
	...
	if(!$post = get_record('ilpconcern_status', 'userid', $user->id, 'live', 1)) {
		$post->status = 0;
	}	
	$options = array(get_string('green', 'ilpconcern'), get_string('amber', 'ilpconcern'), get_string('red', 'ilpconcern'));
	// nkowald - 2010-09-07 - Changed wording to "update progress"
	...
	popup_form ($CFG->wwwroot.'/mod/ilpconcern/concerns_view.php?'.(($courseid > 1)?'courseid='.$courseid.'&amp;' : '').'&amp;userid='.$userid.'&amp;action=updatestatus&amp;studentstatus=', $options, "studentstatus", $post->status, "", "", "", false, 'self', 'Update Progress');
...
function ilpconcern_update_menu($concernpost, $context) {
	...
	$status = optional_param('status', 0, PARAM_INT); //Status	
	...
	// E-learning technologists need to be able to edit/delete tutor reviews
	
	// This max editing time value
	//if((($age < $CFG->maxeditingtime) && $ownpost) || has_capability('moodle/site:doanything', $context)) {
	if((($age < $CFG->maxeditingtime) && $ownpost) || (has_capability('moodle/site:doanything', $context) || (has_capability('mod/ilpconcern:updatestudentstatus', $context))) ) {
