...
if($action == 'updateconcern'){
	...
	$tabs = array();
   	$tabrows = array();
		...
		$tabrows[] = new tabobject('4', "$link_values&amp;status=4", "Subject Reports");
		...
		echo '<div class="addbox">';
			...
			if($CFG->ilpconcern_report4 == 1 && (has_capability('mod/ilpconcern:addreport4', $context) || ($USER->id == $user->id && has_capability('mod/ilpconcern:addownreport4', $context)))) {
				echo '<a href="'.$link_values.'&amp;action=updateconcern&amp;status=3">'.get_string('addconcern', 'ilpconcern', 'Student Progress').'</a>';
			}
			if($CFG->ilpconcern_report4 == 1 && (has_capability('mod/ilpconcern:addreport4', $context) || ($USER->id == $user->id && has_capability('mod/ilpconcern:addownreport4', $context)))) {
				echo '<a href="https://clg.conel.ac.uk/VLE/blocks/lpr/actions/create.php?course_id='.$courseid.'&amp;ilp=1&amp;learner_id='.$user->id.'">Add Subject Report</a>';
			}
