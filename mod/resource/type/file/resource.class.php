    ...
    */
    ...
    /// Form the parse string
        ...
        if ($embedded) {       // Display resource embedded in page
			...
			// nkowald - 2011-01-17 - If resource is a URL then DON'T add the nocache parameter as some webservers spit error when added
			//if (strpos($resource->reference, 'http://') || strpos($resource->reference, 'www.')) {
			// nkowald - 2011-09-16 - some URLs were showing as invalid, use filter_var instead
			if (filter_var($resource->reference, FILTER_VALIDATE_URL)) {
				redirect($fullurl);
			} else {
