...
class auth_plugin_ldap extends auth_plugin_base {
    ...
    function user_login($username, $password) {
        ...
        if ($ldapconnection) {
			...
			// nkowald - 2011-05-04 - Updated to capture error message on fail
            ...
            if ($ldap_login) {
				...
				// nkowald - 2011-05-04 - added LDAP error messages
				$error = ldap_error($ldapconnection);
				$error_msg = '<b>LDAP Error:</b> ' . $error;
				$_SESSION['login']['errormsg'] = $error_msg;
			}
        }
        else {
            $this->ldap_close();
	...
	global $db;
        ...
        switch (strtolower($CFG->dbfamily)) {
            ...
            case 'postgres':
                ...
                $createtemptablesql = 'CREATE TEMPORARY TABLE '. $temptable . ' (username VARCHAR(100), mnethostid BIGINT, PRIMARY KEY (username, mnethostid))';
                break;
            case 'mssql':
                $temptable = '#'. $temptable; /// MSSQL temp tables begin with #
                $droptablesql[] = 'DROP TABLE ' . $temptable; // sql command to drop the table (because session scope could be a problem)
                $bulk_insert_records = 1; // no support for multiple sets of values
                $createtemptablesql = 'CREATE TABLE ' . $temptable . ' (username VARCHAR(100), mnethostid BIGINT, PRIMARY KEY (username, mnethostid))';
                break;
            case 'oracle':
                $droptablesql[] = 'TRUNCATE TABLE ' . $temptable; // oracle requires truncate before being able to drop a temp table
                $droptablesql[] = 'DROP TABLE ' . $temptable; // sql command to drop the table (because session scope could be a problem)
                $bulk_insert_records = 1; // no support for multiple sets of values
                $createtemptablesql = 'CREATE GLOBAL TEMPORARY TABLE '.$temptable.' (username VARCHAR(100), mnethostid NUMBER(10), PRIMARY KEY (username, mnethostid)) ON COMMIT PRESERVE ROWS';
                break;
        }
