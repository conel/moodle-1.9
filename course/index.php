...
/// Move a category to a new parent if required
    if (!empty($move) and ($moveto >= 0) and confirm_sesskey()) {
        if ($cattomove = get_record('course_categories', 'id', $move)) {
			...
			// nkowald - 2010-06-21 - Added this because it needs to update the category path property
			fix_course_sortorder($cattomove->id);
        }
    }
...
/// Recursive function to print all the categories ready for editing
    ...
    if (!empty($category)) {
		...
		// nkowald - 2009-09-01 - Make category stay on one line
		...
		// nkowald - 2009-09-01 - Center align course count
        echo '<td class="count center">'.$category->coursecount.'</td>';

		// nkowald - 2009-09-01 - Make little icons stay on one line
