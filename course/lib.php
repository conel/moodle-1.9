...
// nkowald - 2011-11-10 - Adding new function here to add a new record in the 'mdl_course_idnumber_archive' table
...
function update_course_id_archive($course_obj) {
	...
	if ($course_record = get_record('course_idnumber_archive', 'course_id', $course_obj->id)) {
		...
		// Update record object with new idnumber if it differs
		if ($course_record->$current_year_col != $course_obj->idnumber) {
			$course_record->$current_year_col = $course_obj->idnumber;
			// Record exists: update current years coursecode
			if (!update_record('course_idnumber_archive', $course_record)) {
				echo 'could not update record for course id' . $course_record->id;
			}
		}
	} else {
		// Only create new record if course contains an idnumber
		if ($course_obj->idnumber != '') {
		
			// Create new record object
			$new_course = new stdClass();
			$new_course->course_id = $course_obj->id;
			$new_course->year_0809 = NULL;
			$new_course->year_0910 = NULL;
			$new_course->year_1011 = NULL;
			$new_course->year_1112 = NULL;
			$new_course->year_1213 = NULL;
			$new_course->year_1314 = NULL;
			$new_course->year_1415 = NULL;
			// Update current year's value
			$new_course->$current_year_col = $course_obj->idnumber;

			if (!insert_record('course_idnumber_archive', $new_course, false)) {
				echo 'could not insert record into mdl_course_idnumber_archive with mdl_course id = ' . $course_obj->id;
			}
